...
│  │    • mcp__hydra__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── hydra.log               # Host stdout
│   └── hydra.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.hydra.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'hydra-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your hydra installation
...
Hydra uses a hierarchical memory system based on CLAUDE.md files.
...
Hydra has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__hydra__schedule_task]
        {
...
The `hydra` MCP server is created dynamically per agent call with the current group's context.
...
Hydra runs as a single macOS launchd service.

### Startup Sequence

When Hydra starts, it:
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/hydra.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/hydra.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.hydra.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep hydra` |
| "Claude Code process exited with code 1" | Apple Container failed to start | Check logs; Hydra auto-starts container system but may fail |
...
- `logs/hydra.log` - stdout
- `logs/hydra.error.log` - stderr
