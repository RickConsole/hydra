...
Hydra gives you the core functionality without that mess.
...
- Custom `hydra` MCP server (inside container) provides scheduling tools
...
**Hydra** - Originally named NanoClaw, a reference to Clawdbot (now OpenClaw). Renamed to Hydra to reflect the multi-headed agent architecture.
