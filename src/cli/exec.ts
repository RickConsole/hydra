#!/usr/bin/env node
/**
 * hydra exec — Interactive Claude Code CLI in agent containers
 *
 * Usage:
 *   npm run exec -- main                  # interactive session as main agent
 *   npm run exec -- merlin                # interactive session as merlin
 *   npm run exec -- main --continue       # resume last session
 *   npm run exec -- main --resume <id>    # resume specific session
 */
...
if (!folderName) {
  console.error('Usage: npm run exec -- <agent-folder> [<claude-args>...]');
  console.error('');
