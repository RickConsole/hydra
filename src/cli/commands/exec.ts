...
export async function run(args: string[]): Promise<void> {
  ...
  if (effectiveLlm?.provider === 'litellm' && effectiveLlm.base_url) {
    ...
    envVars.ANTHROPIC_BASE_URL = networkMode === 'host'
      ...
      : rewriteUrl(effectiveLlm.base_url);
  }
  if (effectiveLlm?.api_key) {
    envVars.ANTHROPIC_API_KEY = effectiveLlm.api_key;
  }
  ...
  if (effectiveLlm?.provider === 'litellm' && effectiveLlm.base_url) {
    const apiKey = effectiveLlm.api_key
...
/**
 * Hook script written to the sessions dir and run after each Claude Code response.
 * Queries LiteLLM's spend API and prints token/cost inline (to stderr).
 * Also writes /tmp/litellm-stats.json for the status bar to read.
 */
const LITELLM_HOOK_SCRIPT = `#!/usr/bin/env node
// Auto-generated by Hydra — LiteLLM per-request usage display
// Invoked as a Claude Code Stop hook after each response turn

const base = process.env.ANTHROPIC_BASE_URL;
const key = process.env.LITELLM_API_KEY || process.env.ANTHROPIC_API_KEY;
if (!base || !key) process.exit(0);

// Brief pause for LiteLLM to finish persisting the request log
...
const [logs, info] = await Promise.all([q('/spend/logs?limit=1'), q('/key/info')]);

const log = Array.isArray(logs) ? logs[0] : null;
const totalSpend = info?.info?.spend ?? null;
const maxBudget = info?.info?.max_budget ?? null;

const tokIn  = log?.prompt_tokens     ?? 0;
const tokOut = log?.completion_tokens ?? 0;
const cost   = log?.response_cost     ?? null;

// Write stats file for the status bar to read (non-blocking)
if (log || totalSpend !== null) {
  const stats = {
    last: { tokIn, tokOut, cost, model: log?.model ?? null, ts: Date.now() },
    total: { spend: totalSpend, maxBudget },
  };
  import('fs').then(({ writeFileSync }) => {
    try { writeFileSync('/tmp/litellm-stats.json', JSON.stringify(stats)); } catch {}
  });
}

// Inline display under the response
const parts = [
  \`\${tokIn + tokOut} tok (↑\${tokIn} ↓\${tokOut})\`,
  cost != null   ? \`\$\${cost.toFixed(6)}\`                 : null,
  totalSpend != null
    ? (maxBudget != null
        ? \`total: \$\${totalSpend.toFixed(4)} / \$\${maxBudget}\`
        : \`total: \$\${totalSpend.toFixed(4)}\`)
    : null,
].filter(Boolean).join('  ');

if (parts) process.stderr.write(\`\\x1b[2m  ↳ \${parts}\\x1b[0m\\n\`);
`;
